...
import { SUBJECTS, RESOURCES, type Subject, type Resource } from "./data";
import {
  evaluate,
...
describe("pure-computed ABAC: per-domain tiers + deny overrides", () => {
  it("ALLOWs when clearance, domain tier, need-to-know, and affiliation all pass", () => {
    const d = decide("subj-1", "res-1"); // Dana@A SECRET/DATA:RESTRICTED/[AURORA] -> File Share
  ...
  it("DENYs on the per-domain tier rule even when clearance passes (A7)", () => {
    const d = decide("subj-3", "res-3"); // Lee@C CONFIDENTIAL/COMPUTER:STANDARD -> Dev Jump Host needs PRIVILEGED
  ...
  it("DENYs cross-entity access with no sharing agreement (A<->C absent)", () => {
    const d = decide("subj-1", "res-3"); // Dana@A -> C-owned host
    expect(d.failed).toContain("Affiliation");
  });

  it("ALLOWs cross-entity record release when requester clears and A<->B agreement exists", () => {
    const reqParty = principalFromSubject(subj("subj-1")); // Dana@A requests B's record on Sam
    reqParty.clearance = "TOP_SECRET";
    reqParty.compartments = ["AURORA", "BLACKWING"];
    const d = evaluate(
      reqParty,
      releaseRequirementFor(subj("subj-2"), "ENTITY_B"),
    );
    expect(d.decision).toBe("ALLOW");
  });
