...
## Open Questions — RESOLVED (2026-07-04, no live response within session window; recommended defaults taken)

1. **Does `DatasetAccessDelegate` need an `ORG` delegate-type variant, mirroring `ResourceAccessDelegate`?**
   - **Resolved: `PERSON`-only.** `DatasetAccessDelegate.delegate_type` is always `PERSON` in Phase 13 — matches the grant-based cap mechanism cleanly (an org has no personal `DatasetAccessGrant` to be capped by), and nothing in DATA-DELEG-01 or the discuss-phase interview asked for org-delegates. SPEC.md's own examples are all phrased in terms of a person. Revisit only if a future milestone needs org-level delegation.

2. **What happens when a dataset's linked Applications have divergent effective classifications?**
   - **Resolved: assert-all-share, fail loud on violation (not "take the max").** `effectiveDatasetClassification` may safely read `application_ids[0]`'s classification (matches SPEC.md's stated assumption "since all linked Applications share one classification tier in practice"), guarded by a constructor/seed-validator that asserts ALL of a dataset's linked Applications resolve to the same effective classification — throwing if a future seed ever violates the assumption. This mirrors the codebase's existing fail-closed philosophy (`effectiveClassification`'s missing-Platform throw, DATA-ACCESS-01's non-existent-Application-id throw) rather than introducing a new "highest wins" silent-resolution policy that could mask a genuine seed-integrity bug.
