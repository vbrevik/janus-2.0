...
- [ ] **FED-02**: Inter-entity exchange runs over a typed contract; the viewer sees the message transcript (publish → discover → request → response)
- [ ] **FED-03**: A requester's attributes ride in a signed credential the holder verifies before trusting; forged or untrusted-issuer credentials are rejected (labelled `[MOCK]`)
- [ ] **FED-04**: Detail release is gated by the holder's own ABAC policy — discovery without disclosure (pointer visible, content withheld unless authorized)
