...
- **D-11:** Model the **local authorization lifecycle** (autorisasjon) as a first-class part of the frozen schema, distinct from the external clearance (klarering). Surfaced by a Security-Officer-perspective review (2026-05-21).
  - **Why this matters / the gap:** Today the engine computes ALLOW from clearance + per-domain tier + need-to-know + affiliation (RESEARCH Wave 1). Nothing represents that a unit Security Officer ever **performed and documented an authorization conversation** — the local act that gates access before classified material is touched. AUTH-MODEL §1 scopes *clearance determination* out as external, but **authorization is local, not clearance** — so it does NOT fall under that exclusion and is easy to mis-file as out-of-scope. The SO currently only reacts (`flag` / `securityHold` deny override); there is no positive, documented authorization gate. The SO's whole duty is "performed AND documented", and the schema captures neither.
  ...
  **LOCKED — add to the frozen schema now (seed-only in Phase 1, evaluated in a later phase per D-05):**
  1. `Subject.authorization`: `{ status: AUTHORIZED | WITHDRAWN | PENDING, byRole, conversationDate, validUntil, reauthDue }` — the documented authorization-conversation artifact.
  ...
  3. `AttrOp` additions `AUTHORIZE_SUBJECT` / `WITHDRAW_AUTHORIZATION`, `actor = Security Officer` — event-sourced so Phase 3 replay can answer "was this subject *validly authorized* as of time T". Extends the same R6 logic that forced `SET_REVOKED`/`CLEAR_REVOKED` into the log.
  ...
  **OPEN — resolve in discuss/plan-phase (do not silently default):**
  - **OQ-A (SoD op-set):** Does the Security Officer get a *positive* `AUTHORIZE_SUBJECT` / `WITHDRAW_AUTHORIZATION` action, or remain flag+hold only? In Norwegian practice the SO (*autorisasjonsansvarlig*) IS the one who authorizes; AUTH-MODEL §8 currently gives only the Approver decision authority and leaves the SO flag/hold. Either is defensible but it changes the SO op-set (ROLE-02) and who owns the new ops. Decide before plan-phase locks role gating.
