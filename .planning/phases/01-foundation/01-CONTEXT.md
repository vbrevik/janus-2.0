...
### Personnel Security — Authorization Lifecycle
- **D-11:** Model the **local authorization lifecycle** (autorisasjon) as a first-class part of the frozen schema, distinct from the external clearance (klarering). Surfaced by a personnel-security-perspective review (2026-05-21).
  - **Domain correction — who authorizes:** The authorizing party is the **autoriserende leder / autorisasjonsansvarlig** — the *everyday line manager* who sees and interacts with the employee daily and performs the day-to-day authorization conversation. In the 8 operating roles this maps to **Manager / Supervisor** (scoped, own team), **not** the Security Officer. The **Security Officer** is the specialist (more training, more access) who owns the harder security work — clearance-import mapping, security annotations, risk flagging, and the `securityHold` deny override — and does **not** perform the routine authorization conversation. So the new authorization ops below belong to Manager/Supervisor; the SO role is unchanged by D-11.
  - **Why this matters / the gap:** Today the engine computes ALLOW from clearance + per-domain tier + need-to-know + affiliation (RESEARCH Wave 1). Nothing represents that the autoriserende leder ever **performed and documented an authorization conversation** — the local act that gates access before classified material is touched. AUTH-MODEL §1 scopes *clearance determination* out as external, but **authorization is local, not clearance** — so it does NOT fall under that exclusion and is easy to mis-file as out-of-scope. There is no positive, documented authorization gate today; the schema captures neither the act nor its documentation.
  - **Why now:** D-05 + D-10 freeze the store schema for the whole milestone in Phase 1. These are exactly the kind of forward-only fields D-05 exists to carry; adding them later forces the Phase 3 reshape D-05 was created to prevent.

  **LOCKED — add to the frozen schema now (seed-only in Phase 1, evaluated in a later phase per D-05):**
  1. `Subject.authorization`: `{ status: AUTHORIZED | WITHDRAWN | PENDING, byRole, conversationDate, validUntil, reauthDue }` — the documented authorization-conversation artifact. `byRole` records the autoriserende leder (Manager/Supervisor).
  2. `Subject.clearanceValidUntil` (+ optional `clearanceGrantedBy`) — clearance temporal validity, so an expiry-driven DENY is representable as distinct from a tier/override DENY.
  3. `AttrOp` additions `AUTHORIZE_SUBJECT` / `WITHDRAW_AUTHORIZATION`, `actor = Manager/Supervisor` (autoriserende leder) — event-sourced so Phase 3 replay can answer "was this subject *validly authorized* as of time T". Extends the same R6 logic that forced `SET_REVOKED`/`CLEAR_REVOKED` into the log.
  4. Seed one named contrast actor (e.g. **CA-5 authorization-gap DENY**): clearance ✓, tier ✓, need-to-know ✓, but authorization missing/withdrawn/expired → would DENY for an *authorization* reason, distinct from tier (CA-2) and override (CA-3). Mirrors the existing CA-* fixture pattern; proves the dimension on screen for DEMO-03 once the rule is wired.

  **The clean multi-party SoD this enables** (each party a distinct gate; access = AND of all):
  - **Manager/Supervisor (autoriserende leder):** authorizes the person + records the conversation; can withdraw authorization. Personnel-side gate.
  - **Access Approver / AO:** grants/revokes the domain authorization *attribute*. Access-side decision.
  - **Security Officer:** clearance-import mapping, annotations, risk flag, `securityHold` deny override. Specialist oversight.
  - ALLOW computes only when: authorized (manager) **AND** attribute approved (Approver) **AND** clearance valid **AND** no SO hold.

  **OPEN — resolve in discuss/plan-phase (do not silently default):**
  - **OQ-A (Manager op-set):** Manager/Supervisor's op-set is currently **request-only** (`REQUEST_ATTRIBUTE`, log-only zero mutation — RESEARCH calls this the SoD crux). Adding `AUTHORIZE_SUBJECT` / `WITHDRAW_AUTHORIZATION` makes Manager a *mutating* actor for the **authorization** state. This does NOT break the crux — authorization (a personnel gate) is distinct from the Approver's **attribute grant** — but it is a deliberate ROLE-02 change that must be made consciously. Confirm: Manager gains authorize/withdraw, while still unable to grant attributes. Also note scoping: authorization is scoped to the manager's own team; the demo approximates scoped roles with flat roles (SCOPE-01, deferred) — add a "simplified for demo" note.
  - **OQ-B (evaluate now vs defer):** Is the authorization gate *evaluated* in Phase 1 (adds an `Authorization valid` base rule → diverges from D-01's byte-for-byte engine lift) or seeded-only now and evaluated in Phase 3 alongside the other forward mechanics? **Recommended default: seed now, evaluate Phase 3** — preserves the D-01 verbatim lift and keeps Phase 1 evaluation scope unchanged.
