...
export function SecurityFolderCard({ personId }: { personId: number }) {
  const { data: ndas, isLoading: ndasLoading } = useNDAList({
    person_id: personId,
  });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const res = await apiFetch<{ data: DocumentReference[] }>(
          `/api/document-references?${params.toString()}`,
        );
  ...
  const pendingNDAs = (ndas || []).filter(
    (n: NDA) => n.status === "PENDING" || n.status === "ACTIVE",
  );
            ...
            onClick={async () => {
              try {
                ...
                await createNDA.mutateAsync({
                  ...
                  title: `NDA ${today}`,
                  content:
                    "Please review and sign this Non-Disclosure Agreement.",
                  version: "1.0",
                });
              } catch (e) {
                // no-op
