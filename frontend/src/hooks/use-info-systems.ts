  ...
  return useQuery({
    ...
    queryFn: async () => {
      return apiFetch<PaginatedResponse<InfoSystem>>(
        `/info-systems?page=${page}&per_page=${perPage}`
      )
    },
  })
}
...
export function useInfoSystemDetail(id: number) {
  return useQuery({
    ...
    queryFn: async () => {
      return apiFetch<InfoSystem>(`/info-systems/${id}`)
    },
    enabled: !!id,
  })
}
...
export function useCreateInfoSystem() {
  ...
  return useMutation({
    mutationFn: async (data: CreateInfoSystemRequest) => {
      return apiFetch<InfoSystem>('/info-systems', {
        method: 'POST',
...
export function useDeleteInfoSystem() {
  ...
  return useMutation({
    mutationFn: async (id: number) => {
      return apiFetch(`/info-systems/${id}`, {
        method: 'DELETE',
      })
    },
    onSuccess: () => {
