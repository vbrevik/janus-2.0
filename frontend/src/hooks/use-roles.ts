...
export const rolesKeys = {
  ...
  rolePermissions: (id: number) =>
    [...rolesKeys.all, "permissions", id] as const,
};
...
export function useRolePermissions(roleId: number) {
  return useQuery({
    ...
    queryFn: () => api.get<string[]>(`/api/roles/${roleId}/permissions`),
...
export function useCreateRole() {
  ...
  return useMutation({
    mutationFn: (data: CreateRoleRequest) => api.post<Role>("/api/roles", data),
    onSuccess: () => {
...
export function useSetRolePermissions(roleId: number) {
  ...
  return useMutation({
    mutationFn: (data: SetRolePermissionsRequest) =>
      api.put(`/api/roles/${roleId}/permissions`, data),
    onSuccess: () => {
